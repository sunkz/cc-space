...
final class ExternalEditorDetectorTests: XCTestCase {
    ...
    func test_withTerminalFallbackInsertsTerminalWhenMissing() {
        let iterm = ExternalEditor(
            id: "iterm2",
            displayName: "iTerm2",
        ...
        let terminals = ExternalEditorDetector.withTerminalFallback([iterm])

        XCTAssertEqual(terminals.map(\.id), ["terminal", "iterm2"])
        XCTAssertEqual(terminals.first?.displayName, "终端")
    }

    func test_withTerminalFallbackKeepsDetectedOrderWhenTerminalPresent() {
        let terminal = ExternalEditor(
            id: "terminal",
            displayName: "终端",
        ...
        let terminals = ExternalEditorDetector.withTerminalFallback([terminal, iterm])
