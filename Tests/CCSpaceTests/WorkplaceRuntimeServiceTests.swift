...
private actor WorkplaceRuntimeGitServiceSpy: GitServicing {
    ...
    private(set) var stashPushCalls: [(directory: String, message: String)] = []
    private(set) var popStashCalls: [(index: Int, directory: String)] = []
    private(set) var dropStashCalls: [(index: Int, directory: String)] = []
    private(set) var createLocalBranchCalls: [(branch: String, directory: String)] = []
    private(set) var createBranchFromRevCalls: [(branch: String, rev: String, directory: String)] = []
    private(set) var createBranchFromRemoteCalls: [(branch: String, baseBranch: String, directory: String)] = []
    private(set) var deleteLocalBranchCalls: [(branch: String, directory: String)] = []
    private(set) var deleteRemoteBranchCalls: [(branch: String, directory: String)] = []
    private(set) var abortInterruptedOperationCalls: [String] = []
    var abortInterruptedOperationErrorsByDirectory: [String: Error] = [:]
    var abortInterruptedOperationResultsByDirectory: [String: GitInterruptedOperation] = [:]
    ...
    var stashPushErrorsByDirectory: [String: Error] = [:]
    var popStashErrorsByDirectory: [String: Error] = [:]
    var dropStashErrorsByDirectory: [String: Error] = [:]
    var deleteBranchErrorsByDirectory: [String: Error] = [:]
    var deleteRemoteBranchErrorsByDirectory: [String: Error] = [:]
    ...
    func stashPush(in directory: String, message: String) async throws {
        stashPushCalls.append((directory: directory, message: message))
        if let error = stashPushErrorsByDirectory[directory] {
            throw error
        }
    }
    ...
    func dropStash(at index: Int, in directory: String) async throws {
        dropStashCalls.append((index: index, directory: directory))
        if let error = dropStashErrorsByDirectory[directory] {
            throw error
        }
    }
    ...
    func createLocalBranch(_ branch: String, in directory: String) async throws {
        checkoutCalls.append((branch: branch, directory: directory))
        createLocalBranchCalls.append((branch: branch, directory: directory))
        if let directoryError = checkoutErrorsByDirectory[directory] {
            throw directoryError
        }
        if let checkoutError {
    ...
    func createBranch(_ branch: String, fromRev rev: String, in directory: String) async throws {
        checkoutCalls.append((branch: branch, directory: directory))
        createBranchFromRevCalls.append((branch: branch, rev: rev, directory: directory))
        if let directoryError = checkoutErrorsByDirectory[directory] {
            throw directoryError
        }
        if let checkoutError {
    ...
    func createBranch(fromRemoteBranch branch: String, baseBranch: String, in directory: String) async throws {
        checkoutCalls.append((branch: branch, directory: directory))
        createBranchFromRemoteCalls.append((branch: branch, baseBranch: baseBranch, directory: directory))
        if let directoryError = checkoutErrorsByDirectory[directory] {
            throw directoryError
        }
        if let checkoutError {
    ...
    func deleteLocalBranch(_ branch: String, in directory: String) async throws {
        deleteLocalBranchCalls.append((branch: branch, directory: directory))
        if let error = deleteBranchErrorsByDirectory[directory] {
            throw error
        }
    }
    func deleteRemoteBranch(_ branch: String, in directory: String) async throws {
        deleteRemoteBranchCalls.append((branch: branch, directory: directory))
        if let error = deleteRemoteBranchErrorsByDirectory[directory] {
            throw error
        }
    }

    func abortInterruptedOperation(in directory: String) async throws -> GitInterruptedOperation {
        abortInterruptedOperationCalls.append(directory)
        if let error = abortInterruptedOperationErrorsByDirectory[directory] {
            throw error
        }
        return abortInterruptedOperationResultsByDirectory[directory] ?? .merge
    }
    ...
    func stashPushes() async -> [(directory: String, message: String)] {
        stashPushCalls
    }

    func poppedStashes() async -> [(index: Int, directory: String)] {
        popStashCalls
    }

    func droppedStashes() async -> [(index: Int, directory: String)] {
        dropStashCalls
    }

    func abortedDirectories() async -> [String] {
        abortInterruptedOperationCalls
    }

    func createdLocalBranches() async -> [(branch: String, directory: String)] {
        createLocalBranchCalls
    }

    func createdBranchesFromRev() async -> [(branch: String, rev: String, directory: String)] {
        createBranchFromRevCalls
    }

    func createdBranchesFromRemote() async -> [(branch: String, baseBranch: String, directory: String)] {
        createBranchFromRemoteCalls
    }

    func deletedLocalBranches() async -> [(branch: String, directory: String)] {
        deleteLocalBranchCalls
    }

    func deletedRemoteBranches() async -> [(branch: String, directory: String)] {
        deleteRemoteBranchCalls
    }

    func setDeleteBranchError(_ error: Error?, for directory: String) {
        deleteBranchErrorsByDirectory[directory] = error
    }

    func setDeleteRemoteBranchError(_ error: Error?, for directory: String) {
        deleteRemoteBranchErrorsByDirectory[directory] = error
    }
    ...
    func setStashPushError(_ error: Error?, for directory: String) {
        stashPushErrorsByDirectory[directory] = error
    }

    func setPopStashError(_ error: Error?, for directory: String) {
        popStashErrorsByDirectory[directory] = error
    }

    func setDropStashError(_ error: Error?, for directory: String) {
        dropStashErrorsByDirectory[directory] = error
    }

    func setAbortInterruptedOperationError(_ error: Error?, for directory: String) {
        abortInterruptedOperationErrorsByDirectory[directory] = error
    }
...
final class WorkplaceRuntimeServiceTests: XCTestCase {
    ...
    func test_deleteWorkplaceRestoresSyncStatesWhenLocalRemovalFails() async throws {
        ...
        await XCTAssertThrowsErrorAsync(WorkplaceDeletionError.self) {
            try await service.deleteWorkplace(workplace, removeLocalDirectories: true)
        }
    ...
    func test_switchBranchPersistsFailureWhenStashRestoreFails() async throws {
        ...
        await XCTAssertThrowsErrorAsync(GitWorktreeSafetyError.self) {
            try await service.switchBranch(for: state, in: workplace, to: "release")
        }
    ...
    /// 单仓库夹具:已克隆的工作区 + 注入 spy 的 runtime service。
    private func makeRuntimeFixture(
        uncommittedChanges: Bool = false
    ) async throws -> (
        workplaceStore: WorkplaceStore,
        workplace: Workplace,
        state: RepositorySyncState,
        gitService: WorkplaceRuntimeGitServiceSpy,
        service: WorkplaceRuntimeService
    ) {
        let stores = try makeServiceStores()
        let repositoryStore = stores.repositoryStore
        let workplaceStore = stores.workplaceStore
        ...
        let workplace = try workplaceStore.createWorkplace(
            name: "blog",
            rootPath: stores.workspaceRoot.path,
            selectedRepositories: [repository]
        )
        ...
        let service = WorkplaceRuntimeService(
            workplaceStore: workplaceStore,
            syncCoordinator: SyncCoordinator(gitService: gitService),
            workplaceRootPath: stores.workspaceRoot.path
        )
        return (workplaceStore, workplace, state, gitService, service)
    }

    func test_createBranchCreatesAndSwitchesBranch() async throws {
        let (workplaceStore, workplace, state, gitService, service) = try await makeRuntimeFixture()

        try await service.createBranch(for: state, in: workplace, to: "feature/new")

        let createdBranches = await gitService.createdLocalBranches()
        XCTAssertEqual(createdBranches.count, 1)
        XCTAssertEqual(createdBranches.first?.branch, "feature/new")
        XCTAssertEqual(createdBranches.first?.directory, state.localPath)

        let persistedState = try XCTUnwrap(
            syncState(for: state.repositoryID, workplaceID: workplace.id, in: workplaceStore)
        )
        XCTAssertEqual(persistedState.status, .success)
        XCTAssertNil(persistedState.lastError)
    }

    func test_createBranchWithBaseCreatesFromThatBranchNotHead() async throws {
        let (_, workplace, state, gitService, service) = try await makeRuntimeFixture()

        try await service.createBranch(for: state, in: workplace, to: "feature/new", base: .localBranch("release"))

        let createdWithRev = await gitService.createdBranchesFromRev()
        XCTAssertEqual(createdWithRev.count, 1)
        XCTAssertEqual(createdWithRev.first?.branch, "feature/new")
        XCTAssertEqual(createdWithRev.first?.rev, "release")
        XCTAssertEqual(createdWithRev.first?.directory, state.localPath)
        // 走基线分支路径时不应再触碰"基于 HEAD"的 createLocalBranch。
        let headBranches = await gitService.createdLocalBranches()
        XCTAssertTrue(headBranches.isEmpty)

        // 空白基线视为未指定,回退"基于当前 HEAD"。
        try await service.createBranch(for: state, in: workplace, to: "feature/from-head", base: .localBranch("  "))
        let createdWithRevAfterBlank = await gitService.createdBranchesFromRev()
        XCTAssertEqual(createdWithRevAfterBlank.count, 1)
        let headBranchesAfterBlank = await gitService.createdLocalBranches()
        XCTAssertEqual(headBranchesAfterBlank.count, 1)
        XCTAssertEqual(headBranchesAfterBlank.first?.branch, "feature/from-head")
    }

    func test_createBranchWithRemoteBaseRoutesToRemoteCreation() async throws {
        let (_, workplace, state, gitService, service) = try await makeRuntimeFixture()

        try await service.createBranch(
            for: state,
            in: workplace,
            to: "feature/from-remote",
            base: .remoteBranch("release-202609")
        )

        let createdFromRemote = await gitService.createdBranchesFromRemote()
        XCTAssertEqual(createdFromRemote.count, 1)
        XCTAssertEqual(createdFromRemote.first?.branch, "feature/from-remote")
        XCTAssertEqual(createdFromRemote.first?.baseBranch, "release-202609")
        XCTAssertEqual(createdFromRemote.first?.directory, state.localPath)
        // 远端基线不得走本地 rev / HEAD 两条旧路径。
        let localRevCalls = await gitService.createdBranchesFromRev()
        let headCalls = await gitService.createdLocalBranches()
        XCTAssertTrue(localRevCalls.isEmpty)
        XCTAssertTrue(headCalls.isEmpty)

        // 空白远端基线同样回退"基于当前 HEAD"。
        try await service.createBranch(
            for: state,
            in: workplace,
            to: "feature/from-blank",
            base: .remoteBranch("   ")
        )
        let headBranches = await gitService.createdLocalBranches()
        XCTAssertEqual(headBranches.map(\.branch), ["feature/from-blank"])
    }

    func test_createBranchPersistsLastErrorWhenCreationFails() async throws {
        let (workplaceStore, workplace, state, gitService, service) = try await makeRuntimeFixture()
        await gitService.setCheckoutError(
            WorkplaceRuntimeStubError(message: "branch already exists"),
            for: state.localPath
        )
        ...
        XCTAssertEqual(persistedState.status, .failed)
        XCTAssertEqual(persistedState.lastError, "branch already exists")
    }

    func test_createBranchRejectsEmptyBranchName() async throws {
        let (_, workplace, state, gitService, service) = try await makeRuntimeFixture()

        await XCTAssertThrowsErrorAsync(WorkplaceRuntimeServiceError.self) {
            try await service.createBranch(for: state, in: workplace, to: "   ")
        }

        let createdBranches = await gitService.createdLocalBranches()
        XCTAssertTrue(createdBranches.isEmpty)
    }

    func test_deleteBranchForwardsBranchAndPersistsSuccess() async throws {
        let (workplaceStore, workplace, state, gitService, service) = try await makeRuntimeFixture()
        await gitService.setCurrentBranch("main", for: state.localPath)

        try await service.deleteBranch(for: state, in: workplace, branch: "feature/old")

        let deletedBranches = await gitService.deletedLocalBranches()
        XCTAssertEqual(deletedBranches.count, 1)
        XCTAssertEqual(deletedBranches.first?.branch, "feature/old")
        XCTAssertEqual(deletedBranches.first?.directory, state.localPath)
    ...
    func test_deleteBranchRejectsCurrentBranch() async throws {
        let (_, workplace, state, gitService, service) = try await makeRuntimeFixture()
        await gitService.setCurrentBranch("main", for: state.localPath)

        await XCTAssertThrowsErrorAsync(WorkplaceRuntimeServiceError.self) {
            try await service.deleteBranch(for: state, in: workplace, branch: "main")
        }

        let deletedBranches = await gitService.deletedLocalBranches()
        XCTAssertTrue(deletedBranches.isEmpty)
    }

    func test_deleteBranchPersistsLastErrorWhenDeletionFails() async throws {
        let (workplaceStore, workplace, state, gitService, service) = try await makeRuntimeFixture()
        await gitService.setCurrentBranch("main", for: state.localPath)
        await gitService.setDeleteBranchError(
            WorkplaceRuntimeStubError(message: "delete failed"),
            for: state.localPath
        )
        ...
        XCTAssertEqual(persistedState.status, .failed)
        XCTAssertEqual(persistedState.lastError, "delete failed")
    }

    func test_deleteBranchRejectsEmptyBranchName() async throws {
        let (_, workplace, state, gitService, service) = try await makeRuntimeFixture()

        await XCTAssertThrowsErrorAsync(WorkplaceRuntimeServiceError.self) {
            try await service.deleteBranch(for: state, in: workplace, branch: "   ")
        }
    ...
    func test_deleteRemoteBranchForwardsBranchAndPersistsSuccess() async throws {
        let (workplaceStore, workplace, state, gitService, service) = try await makeRuntimeFixture()

        try await service.deleteRemoteBranch(for: state, in: workplace, branch: "feature/old")

        let deletedBranches = await gitService.deletedRemoteBranches()
        XCTAssertEqual(deletedBranches.count, 1)
        XCTAssertEqual(deletedBranches.first?.branch, "feature/old")
        XCTAssertEqual(deletedBranches.first?.directory, state.localPath)
    ...
    func test_deleteRemoteBranchPersistsLastErrorWhenDeletionFails() async throws {
        let (workplaceStore, workplace, state, gitService, service) = try await makeRuntimeFixture()
        await gitService.setDeleteRemoteBranchError(
            WorkplaceRuntimeStubError(message: "permission denied"),
            for: state.localPath
        )
        ...
        XCTAssertEqual(persistedState.status, .failed)
        XCTAssertEqual(persistedState.lastError, "permission denied")
    }

    func test_stashChangesPushesStashWithManualMessage() async throws {
        let (workplaceStore, workplace, state, gitService, service) = try await makeRuntimeFixture(
            uncommittedChanges: true
        )

        try await service.stashChanges(for: state, in: workplace)

        let stashPushes = await gitService.stashPushes()
        XCTAssertEqual(stashPushes.count, 1)
        XCTAssertEqual(stashPushes.first?.directory, state.localPath)
        XCTAssertEqual(stashPushes.first?.message, "CCSpace manual stash")
    ...
    func test_stashChangesRejectsCleanWorktree() async throws {
        let (workplaceStore, workplace, state, gitService, service) = try await makeRuntimeFixture(
        ...
        do {
            try await service.stashChanges(for: state, in: workplace)
            XCTFail("Expected nothingToStash error")
        } catch {
            XCTAssertEqual(error.localizedDescription, "当前没有可 Stash 的改动")
        }

        let stashPushes = await gitService.stashPushes()
        XCTAssertTrue(stashPushes.isEmpty)
        let persistedState = try XCTUnwrap(
            syncState(for: state.repositoryID, workplaceID: workplace.id, in: workplaceStore)
        )
        XCTAssertEqual(persistedState.status, .failed)
    ...
    func test_popStashRestoresEntryAndPersistsState() async throws {
        ...
        XCTAssertEqual(poppedStashes.first?.index, 1)
        XCTAssertEqual(poppedStashes.first?.directory, state.localPath)
    ...
    func test_popStashPersistsLastErrorWhenPopFails() async throws {
        let (workplaceStore, workplace, state, gitService, service) = try await makeRuntimeFixture()
        await gitService.setPopStashError(
            WorkplaceRuntimeStubError(message: "stash pop conflict"),
            for: state.localPath
        )
        ...
        XCTAssertEqual(persistedState.status, .failed)
        XCTAssertEqual(persistedState.lastError, "stash pop conflict")
    }
    ...
    func test_dropStashRemovesEntry() async throws {
        ...
        XCTAssertEqual(droppedStashes.first?.index, 2)
        XCTAssertEqual(droppedStashes.first?.directory, state.localPath)
    ...
    func test_abortInterruptedOperationReturnsOperationAndPersistsSuccess() async throws {
        let (workplaceStore, workplace, state, gitService, service) = try await makeRuntimeFixture()

        let operation = try await service.abortInterruptedOperation(for: state, in: workplace)

        XCTAssertEqual(operation, .merge)
        let aborted = await gitService.abortedDirectories()
        XCTAssertEqual(aborted, [state.localPath])
    ...
    func test_abortInterruptedOperationPersistsFailureState() async throws {
        let (workplaceStore, workplace, state, gitService, service) = try await makeRuntimeFixture()
        await gitService.setAbortInterruptedOperationError(
            WorkplaceRuntimeStubError(message: "没有可中止的操作"),
            for: state.localPath
        )

        await XCTAssertThrowsErrorAsync(WorkplaceRuntimeStubError.self) {
            _ = try await service.abortInterruptedOperation(for: state, in: workplace)
        }
        ...
        XCTAssertEqual(persistedState.status, .failed)
        XCTAssertEqual(persistedState.lastError, "没有可中止的操作")
    }

    func test_switchRepositoriesToBranchSwitchesAllRepositories() async throws {
        let stores = try makeServiceStores()
        let repositoryStore = stores.repositoryStore
        let workplaceStore = stores.workplaceStore

        try repositoryStore.addRepository(gitURL: "git@github.com:org/blog.git")
        try repositoryStore.addRepository(gitURL: "git@github.com:org/docs.git")
        ...
        let result = try await service.switchRepositoriesToBranch("feature/new", in: workplace)

        XCTAssertEqual(result.successCount, 2)
        XCTAssertEqual(result.failedCount, 0)

        let checkoutCalls = await gitService.checkedOutBranches()
        XCTAssertEqual(checkoutCalls.map(\.branch), ["feature/new", "feature/new"])
    }
...
private func makeBranchStatus(
    ...
    GitBranchStatusSnapshot(
        ...
        aheadCount: hasUnpushedCommits ? 1 : 0
    )
}
