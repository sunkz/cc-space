...
struct OnboardingView: View {
    ...
    enum OnboardingStep: Int, CaseIterable {
        ...
        // 各步标题在对应 step 视图里硬编码(文案随内容排版走),此处不重复提供。
    }
    ...
    private var footerBar: some View {
        HStack(spacing: 12) {
            // 常驻退出入口:外层 sheet 禁用了交互式关闭(Esc / 点击外部),
            // 而首步又要求必须选定根目录才允许前进。没有这里,用户在选目录面板
            // 点「取消」后会同时失去「继续」「跳过」「上一步」,被锁死在引导页。
    ...
    private func chooseRootDirectory() {
        ...
        let path = url.path
        // 与设置页一致:换根与工作区路径迁移一次原子提交,
        // 避免两步写盘中间失败导致记录与设置不一致、被磁盘刷新误删。
        if let errorMessage = settingsStore.saveRootPathAndReturnErrorMessage(
            path,
