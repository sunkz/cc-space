...
final class WorkplaceFormPresentationStateTests: XCTestCase {
    ...
    // MARK: - BranchNameValidation(git check-ref-format 规则)

    func test_branchNameValidationRejectsDoubleSlash() {
        XCTAssertNotNil(BranchNameValidation.validate("a//b"))
        XCTAssertNotNil(BranchNameValidation.validate("feature//x"))
    }

    func test_branchNameValidationRejectsDotPrefixedPathSegments() {
        XCTAssertNotNil(BranchNameValidation.validate("feature/.x"))
        XCTAssertNotNil(BranchNameValidation.validate(".hidden"))
        XCTAssertNotNil(BranchNameValidation.validate("a/..b"))
    }

    func test_branchNameValidationRejectsWhitespaceAndControlCharacters() {
        XCTAssertNotNil(BranchNameValidation.validate("a b"))
        XCTAssertNotNil(BranchNameValidation.validate("a\tb"))
        XCTAssertNotNil(BranchNameValidation.validate("a\u{7F}b"))
        XCTAssertNotNil(BranchNameValidation.validate("a\u{1}b"))
    }

    /// 合法分支名不应被新增规则误伤。
    func test_branchNameValidationAcceptsOrdinaryNames() {
        XCTAssertNil(BranchNameValidation.validate("feature/x.y"))
        XCTAssertNil(BranchNameValidation.validate("release/1.2.3"))
        XCTAssertNil(BranchNameValidation.validate("fix/file-name"))
