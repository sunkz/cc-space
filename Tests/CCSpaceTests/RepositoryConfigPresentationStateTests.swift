...
final class RepositoryBranchSuggestionOrderTests: XCTestCase {
    func test_sortsDedupsAndTrimsLikeNormalization() {
        let ordered = RepositoryBranchSuggestionOrder.ordered(
            branches: [" release", "main", "feature/b", "main", "feature/a", " ", ""],
            defaultBranch: nil
        )
        XCTAssertEqual(ordered, ["feature/a", "feature/b", "main", "release"])
    }

    func test_promotesDefaultBranchToFront() {
        let ordered = RepositoryBranchSuggestionOrder.ordered(
            branches: ["feature/a", "feature/b", "release", "main"],
            defaultBranch: "release"
        )
        XCTAssertEqual(ordered, ["release", "feature/a", "feature/b", "main"])
    }

    func test_missingDefaultBranchKeepsSortedOrder() {
        let ordered = RepositoryBranchSuggestionOrder.ordered(
            branches: ["feature/b", "feature/a"],
            defaultBranch: "not-exist"
        )
        XCTAssertEqual(ordered, ["feature/a", "feature/b"])
    }

    func test_numericNamesUseNaturalOrdering() {
        let ordered = RepositoryBranchSuggestionOrder.ordered(
            branches: ["release/v10", "release/v9", "release/v2"],
            defaultBranch: nil
        )
        XCTAssertEqual(ordered, ["release/v2", "release/v9", "release/v10"])
    }

    func test_emptyInputYieldsEmpty() {
        XCTAssertTrue(RepositoryBranchSuggestionOrder.ordered(branches: [], defaultBranch: "main").isEmpty)
    }
}
