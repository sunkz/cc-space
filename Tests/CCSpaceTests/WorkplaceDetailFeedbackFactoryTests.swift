...
final class WorkplaceDetailFeedbackFactoryTests: XCTestCase {
    ...
    func test_createBranchReturnsSuccessMessage() {
        let feedback = WorkplaceDetailFeedbackFactory.createBranch(
            repositoryName: "blog",
            branch: "feature/new"
        )

        XCTAssertEqual(feedback.style, .success)
        XCTAssertEqual(feedback.message, "已在 blog 创建并切换到 feature/new")
    }

    func test_stashFeedbacksUseSuccessStyle() {
        XCTAssertEqual(
            WorkplaceDetailFeedbackFactory.stashChanges(repositoryName: "blog").message,
            "已 Stash blog 的改动"
        )
        XCTAssertEqual(
            WorkplaceDetailFeedbackFactory.popStash(repositoryName: "blog").message,
            "已恢复 blog 的 Stash"
        )
        XCTAssertEqual(
            WorkplaceDetailFeedbackFactory.dropStash(repositoryName: "blog").message,
            "已删除 blog 的 Stash"
        )
    }
}
